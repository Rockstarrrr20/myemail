...
public class draft extends HttpServlet 
{
            ...
            throws ServletException, IOException 
    {
           ...
           try
           {
            ps=co.prepareStatement("select * from draft where frm=? ");
            ps.setString(1, s1);
             rs=ps.executeQuery();
             pw.print("<html>\n" +
"    <body>\n" +
...
pw.print("<td width=\"60%\" height=\"300\">\n<form action=deld>");
               while(rs.next())
             {
               pw.print("<input type=checkbox name=ch value="+rs.getString(1)+">"+rs.getString(3)+" | "+"<a href=opend?inid="+rs.getString(1)+">"+rs.getString(4)+"</a>");
               pw.print("<hr>");
             }
